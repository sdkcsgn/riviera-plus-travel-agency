...
const UI = {
  ...
  ru: {
    ...
    perPerson: 'за человека / в зависимости от пакета',
    book: 'Забронировать',
    whatsapp: 'Спросить в WhatsApp',
    help: 'Нужна помощь?',
    support: 'Центр поддержки Riviera Plus',
    hours: 'Часы работы',
    hoursValue: '10:00 - 19:00',
...
export default async function TourDetailPage({
  ...
  const suffix =
    lang === 'tr' ? 'Tr' : lang === 'en' ? 'En' : lang === 'de' ? 'De' : 'Ru';

  const title = getDbName(tour, lang);
  const location =
    (tour[`location${suffix}` as keyof typeof tour] as string) || tour.pickup;
  const duration =
    (tour[`duration${suffix}` as keyof typeof tour] as string) || tour.duration;
  const description =
  ...
  const whatsappText = encodeURIComponent(`${title} - ${t.whatsapp}`);
