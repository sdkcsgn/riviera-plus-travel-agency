...
const UI = {
  tr: {
    ...
    contact: '─░leti┼şim',
    back: 'Turlara D├Ân',
    description: 'Tur A├ğ─▒klamas─▒',
    duration: 'S├╝re',
    cancellation: '├£cretsiz ─░ptal',
    cancellationValue: '1 g├╝n ├Âncesine kadar',
    languages: 'Diller',
    region: 'B├Âlge',
    included: 'Tura Dahil Olanlar',
    excluded: 'Tura Dahil Olmayanlar',
    important: '├ûnemli Bilgiler',
    startingFrom: 'Ba┼şlang─▒├ğ fiyat─▒',
    perPerson: 'ki┼şi ba┼ş─▒ / se├ğilen pakete g├Âre',
    book: 'Rezervasyon Yap',
    whatsapp: 'WhatsApp ile Sor',
    help: 'Yard─▒ma m─▒ ihtiyac─▒n─▒z var?',
    support: 'Riviera Plus Yard─▒m Merkezi',
    hours: 'Saatler',
    hoursValue: '10:00 - 19:00',
    whatsappSupport: 'WhatsApp Destek',
    contactForPrice: 'Fiyat i├ğin ileti┼şime ge├ğin',
  },
  en: {
    home: 'Home',
    tours: 'Tours',
  ...
  ru: {
    home: 'ºôº╗º░º▓º¢º░ªÅ',
    tours: 'ºóªâªÇªï',
    categories: 'ºÜº░ªéºÁº│º¥ªÇº©º©',
    contact: 'ºÜº¥º¢ªéº░º║ªéªï',
    back: 'ºØº░ºÀº░º┤ º║ ªéªâªÇº░º╝',
    description: 'ºŞº┐º©ªüº░º¢º©ºÁ ªéªâªÇº░',
    duration: 'ºşªÇº¥º┤º¥º╗ºÂº©ªéºÁº╗ªîº¢º¥ªüªéªî',
    cancellation: 'ºæºÁªüº┐º╗º░ªéº¢º░ªÅ º¥ªéº╝ºÁº¢º░',
    cancellationValue: 'ºöº¥ 1 º┤º¢ªÅ º┤º¥ º¢º░ªçº░º╗º░',
    languages: 'º»ºÀªïº║º©',
    region: 'ºáºÁº│º©º¥º¢',
    included: 'ºÆº║º╗ªÄªçºÁº¢º¥',
    excluded: 'ºØºÁ º▓º║º╗ªÄªçºÁº¢º¥',
    important: 'ºÆº░ºÂº¢º░ªÅ º©º¢ªäº¥ªÇº╝º░ªåº©ªÅ',
    startingFrom: 'ºØº░ªçº░º╗ªîº¢º░ªÅ ªåºÁº¢º░',
    perPerson: 'ºÀº░ ªçºÁº╗º¥º▓ºÁº║º░ / ºÀº░º▓º©ªüº©ªé º¥ªé º┐º░º║ºÁªéº░',
    book: 'ºùº░º▒ªÇº¥º¢º©ªÇº¥º▓º░ªéªî',
    whatsapp: 'ºíº┐ªÇº¥ªüº©ªéªî º▓ WhatsApp',
    help: 'ºØªâºÂº¢º░ º┐º¥º╝º¥ªëªî?',
    support: 'ºĞºÁº¢ªéªÇ º┐º¥º╝º¥ªëº© Riviera Plus',
    hours: 'ºğº░ªüªï ªÇº░º▒º¥ªéªï',
    hoursValue: '10:00 - 19:00',
    whatsappSupport: 'ºşº¥º┤º┤ºÁªÇºÂº║º░ WhatsApp',
    contactForPrice: 'ºúªéº¥ªçº¢º©ªéªî ªåºÁº¢ªâ',
  },
} as const;
...
export default async function TourDetailPage({
  ...
  const suffix = lang === 'tr' ? 'Tr' : lang === 'en' ? 'En' : lang === 'de' ? 'De' : 'Ru';

  const title = getDbName(tour, lang);
  const location =
    (tour[`location${suffix}` as keyof typeof tour] as string) ||
    tour.pickup;
  const duration =
    (tour[`duration${suffix}` as keyof typeof tour] as string) ||
    tour.duration;
  const description =
    (tour[`detailDesc${suffix}` as keyof typeof tour] as string) ||
    getDbDescription(tour, lang);
  ...
  const included = toList(tour[`included${suffix}` as keyof typeof tour]);
  const excluded = toList(tour[`excluded${suffix}` as keyof typeof tour]);
  const important = toList(tour[`important${suffix}` as keyof typeof tour]);
    ...
    <>
      ...
      <div className="topbar">
        <div className="topinner">
          <strong>RIVIERA PLUS ÔÇó ANTALYA</strong>
            ...
            <section className="card content">
              ...
              <div className="facts">
                ...
                <div className="fact">
                  ...
                  <strong>T├╝rk├ğe, English, Deutsch, ºáªâªüªüº║º©º╣</strong>
                </div>
